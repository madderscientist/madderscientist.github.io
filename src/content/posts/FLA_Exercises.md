...
| $q_1$ | | | $q_{back},0,L$ | |  | 加0后回头 |
| $q_{back}$ | $q_{back},0,L$ | $q_{back},1,L$ | $q_2,B,R$ | $q_{back},0,L$ | $q_{back},0,L$ | 到开头<br>顺别负责将X和Y置为0 |
| $q_2$ | $q_3,B,R$ | $q_f,B,R$ | | | | 消去第一个0，开始乘法<br>遇到1说明没0了，结束 | 
| $q_3$ | $q_4,X,R$ | $q_{end},1,R$ | | | | 已读取，准备追加<br>遇到1说明本轮乘法结束<br>先把X和Y置为0再开启下一轮 |
| $q_{end}$ | $q_{end},0,R$ | $q_{end},1,R$ | $q_{back},B,L$ | $q_{end},X,R$ | $q_{end},Y,R$ | 到末尾 |
| $q_4$ | $q_4,0,R$ | $q_5,1,R$ | | | | 找到第一个输出位置 |
| $q_5$ | $q_6,X,R$ | | | | $q_7,Y,L$ | 读取一个输出的0<br>读到Y说明复制完了 |
| $q_6$ | $q_6,0,R$ | | $q_{back2},Y,L$ | | $q_6,Y,R$ | 读一个就尾部复制一个 |
| $q_{back2}$ | $q_{back2},0,L$ | | | $q_5,X,R$ | $q_{back2},Y,L$ | 回到输入的第一个0继续复制 |
